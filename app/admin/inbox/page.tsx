  ...
  return (
    <main className="site">
      ...
      <div className="content admin-content">
        ...
        {error ? <section className="empty-state"><h3>Could not load stories</h3><p>{error.message}</p></section> : stories && stories.length > 0 ? <section className="table-card"><div className="story-table-head"><span>Story</span><span>Status</span><span>Source</span></div>{stories.map((story) => <div className="story-row" key={story.id}><div><Link href={`/admin/inbox/${story.id}`}><strong>{story.title}</strong></Link><small>{story.category || story.country || 'Uncategorized'} · {new Date(story.created_at).toLocaleString()}</small></div><span className={`status status-${story.status}`}>{story.status}</span><a href={story.canonical_url} target="_blank" rel="noreferrer">Original ↗</a></div>)}</section> : <section className="empty-state"><div className="empty-mark">R</div><h3>No stories detected yet.</h3><p>Add and enable a source, then run the ingestion automation. Detected stories will appear here for editorial review.</p><Link className="button-link" href="/admin/sources">Manage sources →</Link></section>}
      </div>
    </main>
  )
}
