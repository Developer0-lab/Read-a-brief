...
export async function POST(request: Request) {
  ...
  try {
    ...
    const briefIds = Array.isArray(body.briefIds) ? [...new Set(body.briefIds.filter((id: unknown): id is string => typeof id === 'string' && id.trim().length > 0).map((id: string) => id.trim()))] : []
